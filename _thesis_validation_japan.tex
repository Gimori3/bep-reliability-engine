...
%
% New BibLaTeX keys (entries PROVIDED in _thesis_validation_japan.bib;
% merge into the thesis .bib or \addbibresource that file):
%   okamura_gounokawa_2025, sako_gounokawa_2019, yabe_levee_committee_2013,
%   tokoro_levee_committee_2017, waseda_gounokawa_dataset_2025
% Existing keys reused: sellmeijer_2011, pol_sie_2024, pol_compgeo_2024,
%   terzaghi1943.
% Figures: figures/validation_shikaga_m4_pattern.png and
% figures/validation_yabe_timeline.png are provided at the repository root
% under figures/ (canonical copies regenerate from scripts/plot_validation_*
% into docs/figures/).
% =========================================================================
...
initiation criterion shows no bias conditional on the investigators'
calibrated FEM heads
(Section~\ref{subsec: Initiation Unbiased Conditional on FEM Heads}); the
...
\subsection{Initiation Is Unbiased Conditional on the Investigators'
Calibrated FEM Heads}
\label{subsec: Initiation Unbiased Conditional on FEM Heads}
...
FEMs rather than by the engine, and the criterion passes all five. No
piezometer record exists anywhere in the case set, so every one of these
checks is conditional on the calibrated seepage models; that conditionality
is carried in this subsection's title deliberately. At Shikaga
...
expected position of the Tokachi production sections is an engineering
judgment along this four-point pattern and is argued in the text, not
...
conservatism cannot contaminate the test, the honest summary is twofold:
realizations that breach do so on the observed clock, while whether a breach
occurs at all within the observed interval depends strongly on the assigned
permeability and is an upper-tail event at the committee's central value.
Quantitatively, under the \textcite{pol_sie_2024} field prior for $C_e$ the
probability of a full breach within the observed interval rises from $0.04$
at the central permeability (where the breaching realizations nevertheless
show a median time of $6.1$~h against the observed $6.3$~h) through $0.41$
at an intermediate value to $0.90$ at the coarse, trench-confirmed
permeability of the pathway actually connected to the river, where the model
is typically faster than observed
(Figure~\ref{fig: yabe timeline test}). The single
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{figures/validation_yabe_timeline.png}
\caption{Forced-clock timeline test at Yabe R7.3k: predicted time from the
committee-anchored initiation (07:00) to the engine breach endpoint and to
the point of no return, across the committee's permeability ladder, against
the observed $6.3$~h initiation-to-breach interval.}
\label{fig: yabe timeline test}
\end{figure}

\subsection{A Directional Pattern of Static Conservatism at the Surviving
Sites}
\label{subsec: Static Comparator Conservative at Surviving Sites}
...
What transfers. The initiation criterion arrives at Tokachi with
substantial field support, short of local piezometry: five FEM-anchored
...
the rate law reproduced the observed breach clock within its prior range,
with breach occurrence itself hinging on the in-layer permeability.
...
the effective-rate reading, not a literal-conduit one.

\paragraph{The principal limitation: the memory model remains untested.}
The compound-event memory model, the mechanism by which pipe damage
persists across peaks and events with zero recovery, is the one core
component of the transient formulation this campaign did not validate. Its
within-event form was merely exercised, not tested: the Yabe record spans
two precursor peaks, but no observable in the set isolates carried-over
damage. Its cross-event form produced the campaign's one documented
disagreement: the Gounokawa boils re-activated at head differences
declining from $6.4$ to approximately $5.8$ and $5.0$~m across the 2018,
2020, and 2021 floods, despite an intervening cutoff wall built to lower
the heads, whereas the engine predicts an unchanged virgin threshold. The
field mechanism is memory held in the blanket's damaged pathways, a state
variable the engine deliberately does not carry; the engine's memory lives
in the pipe length alone. This gap points forward rather than sitting
open: Phase~2 conditions the transient model on the 2016 consecutive-typhoon
survival record, a multi-peak loading whose information content flows
precisely through the within-event memory mechanism, and is therefore the
first quantitative exercise of the component this campaign could not reach.
The Phase~2 results should be read with that dependency explicit.

Taken together, the campaign establishes that the engine's initiation
criterion (conditional on the investigators' calibrated heads) and its
progression race condition are sound in the high-gradient Japanese setting
on the evidence available, that its known conservatisms are localized (head
translation) and bounded (gate-only, shoulder-concentrated), and that the
static comparator's conservatism, the premise of this thesis's central bias
question, is visible in the field wherever the record permits a check. Its
principal limitation is equally explicit: the memory model enters Phase~2
unvalidated by field observation, and Phase~2 is the exercise that gap
calls for.
