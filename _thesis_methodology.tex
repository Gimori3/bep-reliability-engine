...
The seventh random variable, the empirical erosion coefficient $C_e$, requires specific methodological justification. The Pol (2024) ODE was derived from a sediment transport formulation calibrated against small-scale and large-scale piping experiments, all conducted under laminar pipe flow conditions ($\mathrm{Re} < 2100$). Recent Japanese centrifuge modeling demonstrates that prototype-scale pipe flow frequently transitions to turbulent flow, introducing hydraulic friction that slows progression relative to laminar predictions \parencite{Okamura2022, Okamura2025_prediction}. The conservative laminar-flow assumption therefore biases the deterministic application of the Pol ODE toward overestimating progression rates at field scale. Rather than modifying the ODE itself with empirically unvalidated turbulence corrections, this study treats the calibration uncertainty in $C_e$ as an explicit component of the stochastic parameter space. This methodological choice is critical for the coherence of the Phase~2 Bayesian filtering: if $C_e$ were held deterministic, the filter could constrain $k_\mathrm{aquifer}$, $D_{bl}$, and the other geotechnical variables, but could not adjust the single parameter most directly encoding the laminar-to-turbulent progression rate discrepancy. Promoting $C_e$ to a stochastic variable resolves this consistently and allows the 2016 survival observation to constrain laminar-flow conservatism through the same mechanism that constrains the geotechnical priors.

The prior distribution for $C_e$ is specified as $\mathrm{Lognormal}$ with mean $0.014$ (corresponding to the Pol large-scale FPH calibration) and coefficient of variation $0.50$, spanning the small-scale calibration range of approximately $0.007$ to $0.030$ reported by \textcite{pol_compgeo_2024}.
...
As a supplementary updating step, the well-documented peak water levels from the September 2011 flood event are incorporated as an additional survival constraint, applied sequentially to the posterior parameter set derived from the 2016 filtering. This provides an independent empirical anchor at a different position in the hydrograph severity spectrum and further constrains the tails of the posterior parameter distributions.
