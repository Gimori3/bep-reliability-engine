...
COMPANION_EXCLUSIONS: dict[str, str] = {
    ...
    "scripts/conductivity_annualisation_study.py": (
        ...
        "the substantive ground for excluding it is unchanged. Widened again "
        "2026-08-21, when the --side posterior axis landed: on that side it "
        "additionally consumes 32 arm POSTERIORS produced by "
        "scripts/conductivity_posterior_replay.py, which are gitignored for the "
        "same reason and absent for the same reason. The exclusion ground is "
        "unchanged and now covers two artifact families rather than one. Cheap "
        "in itself (recorded 8.0 s per reading) but it raises FileNotFoundError "
        "on a missing arm, so on any machine without those gitignored sweeps it "
        ...
        "rq4_annual.csv over 228 rows x 20 fields, now once per reading and "
        "once per side) still has no gate-only entry point; adding one is the "
        "cheap way to make it campaign-runnable if that is ever wanted. Both "
        "its figures ARE re-drawn by the campaign, in the figures stage under "
        "G7; the posterior side deliberately renders none."
    ),
    "scripts/conductivity_posterior_replay.py": (
        "posterior-side conductivity replay driver (2026-08-21, companion "
        "record conductivity-bracket-posterior-side.md). Excluded on the same "
        "ground as the study it feeds, and one of its own: it consumes the 32 "
        "gitignored ADR-0048 arm sweeps and would raise FileNotFoundError "
        "without them, and it is 32 Phase 2 replays at roughly 70 to 140 s "
        "each, so about an hour of campaign time for a companion the campaign "
        "does not deliver. It re-implements no part of the update: it calls "
        "bayesian_reliability_updating.pipeline.run_survival_update with "
        "settings read from the production sidecar and refuses on drift, so "
        "the acceptance rule it exercises IS the one the campaign's own "
        "phase2_baseline stage exercises under G2."
    ),
