...
- **Phase 2 replay timestep.** The ADR-0022 native/2 = 1800 s decision for the per-realization 2016 replay predates the ADR-0030 evidence; the row-wise Δt ladder must be re-run on the 2016 record before Phase 2 relies on it (per-row Accept–Reject decisions are more Δt-sensitive than population-level P_f).
- **KP58.8 r_e-halved QA member (registered, not yet run).** The next production re-sweep must include one KP58.8 sensitivity member with r_e → r_e/2, converting the Japanese case-validation shoulder bound (M4 over-translation 1.15–2.7× at FEM-calibrated sites) into a measured ΔP_f per conditioning level (ADR-0032 scope amendment; `docs/validation/shikaga-case.md` §3; `scripts/run_sweep.py` header).
...
| Timestep | Δt = native/16 = 225 s for Phase 1 (ADR-0030, superseding ADR-0022's native 3600 s on the ADR-0026/0027 material rate-law changes); Phase 2 replay at 1800 s (open re-check per ADR-0030); kernel Δt/2 guard 600↔300 s |
