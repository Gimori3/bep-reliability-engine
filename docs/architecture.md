  ...
  8. For each timestep t_k:
       a. h_aq(t_k)        = z_toe + r_e · (h(t_k) − z_toe)
                              [instantaneous form — the executed ADR-0032 diagnostic
                               retained it (Π ≈ 0.01 ≪ 0.10); the dormant lag hook
...
| Aquifer-lag activation | **Instantaneous retained, on evidence** (ADR-0032, executed 2026-07-11): pre-registered Π = τ_aq/T_rise vs Π\* = 0.10 at upper-bound S_s = 1×10⁻⁴ m⁻¹ gives Π ≈ 0.010–0.012 at the τ_aq-bounding pair KP58.8/KP60.0 (~10× margin; S_s did not bind); recorded per run in `metadata['aquifer_response']`; scope: elastic leaky-confined, channel-connected sections only |
